...
const { sendTranscriptionToUI } = require('../utils/ui-bridge');

function extractUsername(sipAddress) {
  ...
  if (call_sid_b) {
    translateText(logger, transcript, recognizer_a.language, recognizer_b.language)
      .then((translation) => {
        ...
        sendTranscriptionToUI(logger, {
          agent: extractUsername(TARGET_AGENT[0].name),
          channel: 1,
